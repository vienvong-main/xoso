<div class="container">
    <nav class="navbar navbar-static-top navbar-inverse">
        <div class="container-fluid">
            <div class="navbar-header">
                <button class="navbar-toggle collapsed" aria-controls="navbar" aria-expanded="false"
                        data-target="#navbar"
                        data-toggle="collapse" type="button">
                    <span class="sr-only">Menu</span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </button>
                <a id="logo-text" class="navbar-brand" href="/">XSBacNam</a>
                <span id="logo-text-sub" class="navbar-brand">.com</span>
            </div>
            <div id="navbar" class="collapse navbar-collapse navbar-right">
                <ul class="nav navbar-nav">
                    <li id="navbar_today" class="active">
                        <a href="/today">Hôm nay </a>
                    </li>
                    <li id="navbar_sokq" class="">
                        <a href="/sokq">Sổ KQ</a>
                    </li>
                    <li id="navbar_quaythu" class="">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
                           aria-haspopup="true" aria-expanded="true">Quay Thử <span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="/quaythu?region=mb">Miền Bắc</a></li>
                            <li><a href="/quaythu?region=mt">Miền Trung</a></li>
                            <li><a href="/quaythu?region=mn">Miền Nam</a></li>
                        </ul>
                    </li>

                    <li id="navbar_xsmb" class="">
                        <a href="/xsmb">XSMB</a>
                    </li>
                    <li id="navbar_xsmt" class="">
                        <a href="/xsmt">XSMT</a>
                    </li>
                    <li id="navbar_xsmn" class="">
                        <a href="/xsmn">XSMN</a>
                    </li>
                    <li id="navbar_xsmn" class="">
                        <a href="/xsmn">Bóng Đá</a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
</div>